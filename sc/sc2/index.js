...
if (!fetched) {
  var PART_NAMES = [
    'https://cdn.jsdelivr.net/gh/UGBONTOP/sonic-1-jsdlivr@main/sonic2/index.data.part0',
    'https://cdn.jsdelivr.net/gh/UGBONTOP/sonic-1-jsdlivr@main/sonic2/index.data.part1',
    'https://cdn.jsdelivr.net/gh/UGBONTOP/sonic-1-jsdlivr@main/sonic2/index.data.part2',
  ];
        var partBuffers = [null, null, null];
        var partsLoaded = 0;
        PART_NAMES.forEach(function(partName, i) {
          fetchRemotePackage(partName, REMOTE_PACKAGE_SIZE / PART_NAMES.length, function(data) {
            partBuffers[i] = data;
            partsLoaded++;
            if (partsLoaded === PART_NAMES.length) {
              var totalLen = partBuffers.reduce(function(s, b) { return s + b.byteLength; }, 0);
              var combined = new Uint8Array(totalLen);
              var offset = 0;
              partBuffers.forEach(function(b) { combined.set(new Uint8Array(b), offset); offset += b.byteLength; });
              var combinedBuffer = combined.buffer;
              if (fetchedCallback) { fetchedCallback(combinedBuffer); fetchedCallback = null; }
              else { fetched = combinedBuffer; }
            }
          }, handleError);
        });
      }
